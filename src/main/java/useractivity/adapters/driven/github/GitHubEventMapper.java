...
public class GitHubEventMapper {
    ...
    private PullRequestEvent mapToPullRequestEvent(String id, User actor, Repository repo, ZonedDateTime createdAt, JsonNode payload) {
        PullRequestAction action = PullRequestAction.fromString(payload.get("action").asText());
        PullRequest pullRequest = mapToPullRequest(payload.get("pull_request"));
        return new PullRequestEvent(id, actor, repo, createdAt, action, pullRequest);
    }
